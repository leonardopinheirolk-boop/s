...
# CSS global: mantém o conteúdo principal com texto escuro e preserva o contraste nos blocos coloridos
st.markdown("""
<style>
...
[data-testid="stAppViewContainer"] div:not([style*="color:"]) {
    color: #111827;
}
...
def mapear_bimestre(periodo: str) -> int | None:
    ...
    if not isinstance(periodo, str):
        return None
    p = periodo.lower()
    if "primeiro" in p or "1º" in p or "1o" in p or "1°" in p:
        return 1
    if "segundo" in p or "2º" in p or "2o" in p or "2°" in p:
        return 2
    if "terceiro" in p or "3º" in p or "3o" in p or "3°" in p:
        return 3
    if "quarto" in p or "4º" in p or "4o" in p or "4°" in p:
        return 4
    return None

def rotulo_bimestre(valor):
    mapa = {1: "1º Bimestre", 2: "2º Bimestre", 3: "3º Bimestre", 4: "4º Bimestre"}
    return mapa.get(valor)

def preparar_coluna_bimestre(df):
    """Garante uma coluna textual Bimestre com base em Periodo."""
    df = df.copy()
    if "Bimestre" in df.columns:
        serie = df["Bimestre"]
        if pd.api.types.is_numeric_dtype(serie):
            df["Bimestre"] = serie.apply(rotulo_bimestre)
        else:
            df["Bimestre"] = serie.astype(str).str.strip()
    elif "Periodo" in df.columns:
        df["Bimestre"] = df["Periodo"].apply(mapear_bimestre).apply(rotulo_bimestre)
    else:
        df["Bimestre"] = np.nan
    return df
...
def calcula_indicadores(df):
    """
    Cria um dataframe por Aluno-Disciplina com métricas robustas,
    mesmo quando o filtro traz somente parte dos bimestres.
    """
    df = df.copy()
    if "Periodo" in df.columns:
        df["Bimestre"] = df["Periodo"].apply(mapear_bimestre)
    elif "Bimestre" in df.columns:
        if not pd.api.types.is_numeric_dtype(df["Bimestre"]):
            df["Bimestre"] = df["Bimestre"].apply(mapear_bimestre)
    else:
        df["Bimestre"] = np.nan
    ...
    if coluna_aluno is None:
        raise KeyError("Nenhuma coluna de aluno encontrada para calcular os indicadores.")
    ...
    rename_cols = {b: f"N{b}" for b in [1, 2, 3, 4] if b in pivot.columns}
    ...
    pivot["Soma12"] = pivot[["N1", "N2"]].fillna(0).sum(axis=1)
    pivot["Media12"] = pivot[["N1", "N2"]].mean(axis=1)
    pivot["Media123"] = pivot[["N1", "N2", "N3"]].mean(axis=1)
    pivot["MediaFinal"] = pivot[["N1", "N2", "N3", "N4"]].mean(axis=1)
    ...
    pivot["Classificacao"] = [
        ...
        for _n1, _n2, _m12 in zip(pivot["N1"], pivot["N2"], pivot["Media12"])
    ]
...
# Garantir a coluna Bimestre para filtros e exportações
df = preparar_coluna_bimestre(df)
bimestres = [b for b in ["1º Bimestre", "2º Bimestre", "3º Bimestre", "4º Bimestre"] if b in df["Bimestre"].dropna().astype(str).unique().tolist()]

st.sidebar.markdown("""
<div style="background: linear-gradient(135deg, #dbeafe, #bfdbfe); border-radius: 6px; padding: 8px 12px; margin: 6px 0; box-shadow: 0 1px 4px rgba(59, 130, 246, 0.12); border-left: 3px solid #2563eb;">
    <h3 style="color: #1d4ed8; margin: 0; font-size: 1em; font-weight: 600;">Bimestre</h3>
</div>
""", unsafe_allow_html=True)
col_b1, col_b2 = st.sidebar.columns(2)
with col_b1:
    if st.button("Todos", key="btn_todos_bim", help="Selecionar todos os bimestres"):
        st.session_state.bimestres_selecionados = bimestres
with col_b2:
...
bimestre_sel = st.sidebar.multiselect(
    ...
    help="As tabelas, gráficos e exportações usarão somente os bimestres selecionados"
)
st.session_state.bimestres_selecionados = bimestre_sel
